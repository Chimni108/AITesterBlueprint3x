...
## Goal
Given a list of generated test cases and the source JIRA issue key the user entered, create one **Sub-task** issue per test case, parented to the source issue, and return the resulting list of created JIRA issue keys/URLs per the "JIRA Write-Back Output" schema in `gemini.md`.
...
- `parentKey`: the source issue (the same key the user entered to generate the test cases) — the project key for the new Sub-tasks is derived from this (e.g. `SCRUM-6` → project `SCRUM`).
...
- If the JIRA project does not have "Sub-task" enabled, the create call fails per-item and is surfaced via the `failed` list — no silent fallback to a different issue type.
