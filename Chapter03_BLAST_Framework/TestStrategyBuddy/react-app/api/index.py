...
def _validate_payload(body, *, require_test_cases=False):
    jira = body.get("jira") or {}
    groq = body.get("groq") or {}
    issue_key = body.get("issueKey")

    required = [jira.get("baseUrl"), jira.get("email"), jira.get("apiToken"), issue_key]
    if not require_test_cases:
        required += [groq.get("apiKey"), groq.get("model")]
